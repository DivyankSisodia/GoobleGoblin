...
class _WalletHistoryScreenState extends ConsumerState<WalletHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    List<Payment> filteredPayments = payments.where((payment) {
      ...
      bool isIncome = category.label.toLowerCase().contains('income') || 
                      category.label.toLowerCase().contains('salary') ||
                      (payment.note?.toLowerCase().contains('salary') ?? false) ||
                      (payment.note?.toLowerCase().contains('deposit') ?? false);
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: isSelected 
                    ? const LinearGradient(colors: [Color(0xFFFF00FF), Color(0xFFE91E63)])
                    : null,
  ...
  Widget _buildDateGroup(String date, List<Payment> payments, List<Category> categories, List<BankCard> cards) {
    ...
    double dailyTotal = payments.fold(0, (sum, p) {
      final cat = categories.firstWhere((c) => c.id == p.categoryId, orElse: () => Category(label: '', icon: ''));
      bool isIncome = cat.label.toLowerCase().contains('income') || 
                      cat.label.toLowerCase().contains('salary') ||
                      (p.note?.toLowerCase().contains('salary') ?? false);
              ...
              Container(
                ...
                child: category.icon.isNotEmpty 
                    ? (category.icon.endsWith('.png') 
                        ? Image.asset(category.icon, color: Colors.white, colorBlendMode: BlendMode.srcIn) 
          ...
          Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                  color: isIncome ? const Color(0xFF00FF9D) : const Color(0xFFFF4B4B),
                  fontSize: 17,
                  fontWeight: FontWeight.bold,
                  fontFamily: GoogleFonts.montserrat().fontFamily,
                ),
              ...
              Text(
                payment.createdAt != null 
                    ? DateFormat('hh:mm a').format(DateTime.parse(payment.createdAt!))
